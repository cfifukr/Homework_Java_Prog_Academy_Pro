			...
			while (true) {
				if (check == false){
					...
					if(login.toUpperCase().compareTo("ALL") != 0){
						user = new User(login, Status.ONLINE, id);
				...
				if (user != null && user.status == Status.ONLINE){
					System.out.println("To become offline - use comand '/off'" +
							"\nif u dont want just press enter ");
					if(scanner.nextLine().compareTo("/off") == 0){
						for(User user1 : UserList.getInstance()){
				...
				}else if (user != null && user.status == Status.OFFLINE){
					System.out.println("To become online - use comand '/on'" +
							"\nif u dont want just press enter ");
					if(scanner.nextLine().compareTo("/on") == 0){
						for(User user1 : UserList.getInstance()){
							if (user1.getId() == user.getId()){
								user1.goOnline();
